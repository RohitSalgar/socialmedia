...
/** Calculating the Actual Booking fee for the Booking */
const actualBookingFee = (cbm, weight) => {
	if (weight >= THRESHOLD_GROSSWEIGHT) {
		return BOOKING_FEES * Math.ceil(weight / THRESHOLD_GROSSWEIGHT);
	} else if (cbm >= THRESHOLD_CBM) {
		return BOOKING_FEES * Math.ceil(cbm / THRESHOLD_CBM);
	} else {
		return BOOKING_FEES;
	}
};
